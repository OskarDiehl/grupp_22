...
public class EnemyTest {
    ...
    void getElementFromConstructorArgument() {
        ...
        Character character = new Enemy(element, 2, new Room(new Player("Malin", new WaterElement(1), Role.Tank)));
        assertEquals(element, character.getMainElement());
    }

    @Test
    void getLevelFromConstructorArgument() {
        Enemy enemy = new Enemy(new FireElement(2), 2, new Room(new Player("Malin", new WaterElement(1), Role.Tank)));
        assertEquals(2, enemy.getLevel());
    }
    
    @Test
    void getIAEFromIllegalLevelConstructorArgument() {
        assertThrows(IllegalArgumentException.class, () -> new Enemy(new FireElement(2), 4, new Room(new Player("Malin", new WaterElement(1), Role.Tank))));
    }

    @Test
    void getRoomFromConstructorArgument() {
        Room room = new Room(new Player("Malin", new WaterElement(1), Role.Tank));
    ...
    void generateEnemyStatsForLevel1(){
        ...
        Enemy enemy = new Enemy(new FireElement(2), 1, new Room(new Player("Malin", new WaterElement(1), Role.Tank)));
    ...
    void generateEnemyStatsForLevel3(){
        ...
        Enemy enemy = new Enemy(new FireElement(2), 3, new Room(new Player("Malin", new WaterElement(1), Role.Tank)));
        assertEquals(stats.toString(), enemy.getStats().toString());
    }
    ...
    void removeIfDeadWhenIsDead() {
        ...
        int hp = enemy.getHP();
        enemy.getStats().changeCurrentHP(hp * -1);
        enemy.removeIfDead();
